...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	for nProcesses := 0; nProcesses < concurrency; nProcesses++ {
		go JobWorker(j.MessageChannel, j.JobReturnChannel, jobService, j.Domain, nProcesses)
	}
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("MessageID: %v. Error during job: %v with video: %v. Error: %v", jobResult.Message.DeliveryTag, jobResult.Job.ID, jobResult.Job.VideoID, jobResult.Error.Error())
	} else {
